...
<#
.SYNOPSIS
הצגת חדשות Radio-Canada
.DESCRIPTION
הפונקציה נוצרה להצגת חדשות כאובייקטי PowerShell. ניתן לשלוף אותן ב-Universal Dashboard, למשל להצגת חדשות בלוח מחוונים בטלוויזיות.
.PARAMETER Information
הצגת מידע לפי הבחירה
.PARAMETER Thematiques
הצגת נושאים לפי הבחירה
.PARAMETER Sports
הצגת ספורט לפי הבחירה
.PARAMETER Arts
הצגת אמנויות לפי הבחירה
.PARAMETER Regions
הצגת אזורים לפי הבחירה
.PARAMETER Autres
הצגת קטגוריית אחר לפי הבחירה
.PARAMETER HTML
מציג את החדשות בסיכום HTML שנפתח בדפדפן.
.LINK
Sebastien Maltais
...
# שאילתת שרת Radio-Canada
$RSS_Query = Invoke-RestMethod -Uri $RSS

# חילוץ נתונים מהזנת ה-RSS
 [array]$nouvelle = @(
                     $RSS_Query | Foreach{
...
# תמונת כותרת HTML של Radio-Canada
