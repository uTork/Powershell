...
.DESCRIPTION
La fonction a été créée pour afficher les nouvelles dans des objets PowerShell. Elles peuvent être récupérées par Universal Dashboard par exemple pour afficher les nouvelles sur un Dashboard sur des téléviseurs.
.PARAMETER Information
Affiche les informations selon la sélection
.PARAMETER Thematiques
Affiche les thématiques selon la sélection
.PARAMETER Sports
Affiche les sports selon la sélection
.PARAMETER Arts
Affiche les arts selon la sélection
.PARAMETER Regions
Affiche les régions selon la sélection
.PARAMETER Autres
...
# Interroger le serveur de Radio-Canada
$RSS_Query = Invoke-RestMethod -Uri $RSS

# Extraire les données du fil RSS
 [array]$nouvelle = @(
                     $RSS_Query | Foreach{
...
# Image d'en-tête HTML Radio-Canada
