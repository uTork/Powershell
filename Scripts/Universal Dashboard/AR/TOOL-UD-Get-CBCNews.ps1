...
# عنوان RSS لـ The National
$THENATIONAL = @{
                    In_Depth_Reports = "https://rss.cbc.ca/lineup/thenational.xml"
                 }

# تحديد موجز RSS للأخبار العامة
if($GENERAL_NEWS -NE ""){
                         $RSS = $GENERALNEWS."$GENERAL_NEWS"                          
                        }

# تحديد موجز RSS للأخبار الإقليمية
if($REGIONAL_NEWS -ne ""){                       
                         $RSS = $REGIONALNEWS."$REGIONAL_NEWS"
                         }

# تحديد موجز RSS لـ The National
if($THE_NATIONAL -ne ""){
