...
<#
.SYNOPSIS
صورة NASA لليوم / يتطلب الإنترنت
.DESCRIPTION
فتح صورة NASA لليوم في صفحة HTML. استخراج عنوان URL. حفظ الصورة في مجلد
.PARAMETER HTML
فتح الصورة في متصفح HTML.
.PARAMETER IMAGEURL
عرض عنوان URL للصورة
.PARAMETER SAVEPATH
حفظ الصورة في مجلد.
.LINK
Sebastien Maltais
...
# احصل على مفتاح API مجاني من موقع NASA https://api.nasa.gov/index.html#apply-for-an-api-key
...
# إخراج عنوان URL عالي الدقة للصورة فقط
if($image_url -eq $true){Write-Output $hdurl}

# إنشاء صفحة HTML تحتوي على الصورة
if($html -eq $true){
