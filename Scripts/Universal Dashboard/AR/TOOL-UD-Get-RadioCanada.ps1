...
<#
.SYNOPSIS
عرض أخبار Radio-Canada
.DESCRIPTION
أُنشئت هذه الدالة لعرض الأخبار ككائنات PowerShell. يمكن استرجاعها بواسطة Universal Dashboard لعرض الأخبار على لوحة على أجهزة التلفاز مثلًا.
.PARAMETER Information
عرض المعلومات حسب التحديد
.PARAMETER Thematiques
عرض الموضوعات حسب التحديد
.PARAMETER Sports
عرض الرياضة حسب التحديد
.PARAMETER Arts
عرض الفنون حسب التحديد
.PARAMETER Regions
عرض المناطق حسب التحديد
.PARAMETER Autres
عرض فئة أخرى حسب التحديد
.PARAMETER HTML
يعرض الأخبار في ملخص HTML يفتح في المتصفح.
.LINK
Sebastien Maltais
...
# استعلام خادم Radio-Canada
$RSS_Query = Invoke-RestMethod -Uri $RSS

# استخراج البيانات من موجز RSS
 [array]$nouvelle = @(
                     $RSS_Query | Foreach{
...
# صورة ترويسة HTML لـ Radio-Canada
