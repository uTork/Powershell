...
<#
.SYNOPSIS
Зображення NASA дня / Потрібен інтернет
.DESCRIPTION
Відкрити зображення NASA дня на HTML-сторінці. Витягти URL. Зберегти зображення в папку
.PARAMETER HTML
Відкрити зображення в HTML-браузері.
.PARAMETER IMAGEURL
Показати URL зображення
.PARAMETER SAVEPATH
Зберегти зображення в папку.
.LINK
Sebastien Maltais
...
# Отримайте безкоштовний особистий API-ключ на сайті NASA https://api.nasa.gov/index.html#apply-for-an-api-key
...
# Вивести об’єкт зображення дня
if($savepath -eq "" -and $IMAGE_URL -ne $true -and $html -ne $true){$NasaQuery}

# Зберегти зображення у файл на жорсткому диску
if($SAVEPATH -ne ""){
...
# Вивести лише HD URL зображення
if($image_url -eq $true){Write-Output $hdurl}

# Створити HTML-сторінку із зображенням
if($html -eq $true){
