...
<#
.SYNOPSIS
Показати новини Radio-Canada
.DESCRIPTION
Цю функцію створено для відображення новин як об’єктів PowerShell. Їх можна отримувати через Universal Dashboard, наприклад для показу новин на панелі на телевізорах.
.PARAMETER Information
Показати інформацію згідно з вибором
.PARAMETER Thematiques
Показати тематики згідно з вибором
.PARAMETER Sports
Показати спорт згідно з вибором
.PARAMETER Arts
Показати мистецтва згідно з вибором
.PARAMETER Regions
Показати регіони згідно з вибором
.PARAMETER Autres
Показати категорію Інше згідно з вибором
.PARAMETER HTML
Показує новини в HTML-зведенні, яке відкривається в браузері.
.LINK
Sebastien Maltais
...
# Запит до сервера Radio-Canada
$RSS_Query = Invoke-RestMethod -Uri $RSS

# Витягнути дані з RSS-каналу
 [array]$nouvelle = @(
                     $RSS_Query | Foreach{
...
# Відобразити як об’єкт
if($html -ne $true){$nouvelle}

# Відобразити в HTML
if($html -eq $true){

# Зображення заголовка HTML Radio-Canada
